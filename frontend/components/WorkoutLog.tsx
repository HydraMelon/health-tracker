...
export default function WorkoutLog({ muscleGroups, onLogSession }: Props) {
  ...
  if (muscleGroups.length === 0) {
    return (
      <div className="bg-white border border-gray-100 rounded-xl p-10 text-center">
        <p className="text-sm font-medium text-gray-500 mb-1">No muscle groups yet</p>
        <p className="text-xs text-gray-300 mb-5">Add one to start logging your workouts</p>
        <button
          onClick={() => onLogSession?.()}
          className="bg-teal-700 hover:bg-teal-800 text-teal-50 text-sm font-medium px-5 min-h-[44px] rounded-lg"
        >
          Get started
        </button>
      </div>
  ...
  const selected = muscleGroups[Math.min(selectedIndex, muscleGroups.length - 1)]
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={() => onLogSession?.(selected.id)}
